...
def getMoves():
    ...
    #numMunchersToDeploy = max(remainingStuff[0] / 2,1)
    nodesToCheck = []
    if len(otherLiveMunchers) > 0:
        nodesToCheck = getOpponentsNeighborNodes()
        if len(nodesToCheck) < numMunchersToDeploy:
            components = getNLargestConnectedComponents(numMunchersToDeploy - len(nodesToCheck), n_comp, labels)
...
def getNumMunchersToDeploy():
    ...
    if len(otherLiveMunchers) == 0:
        ...
        if len(liveMunchers) < len(otherLiveMunchers):
