...
class graph;
int make_move(int& move, graph& g, string& p, int depth, string&answer, int& answercnt);
...
void game_parser(graph& g){
    ...
    else
    {
		...
		if(g.first == 1){		
			g.first = 0;
			make_move(move, g, p,5,answer, answercnt);
			make_move(move, g, p,30,answer, answercnt);
		}
		else
		for(int i = 0;i<g.remain_muncher_num-g.ad_remain_muncher_num ; ++i){
			if(g.ad_remain_muncher_num <= 2)
				make_move(move, g, p, 10, answer, answercnt);
			else
				make_move(move, g, p, 5, answer, answercnt);
		}
		//Blocking opponent if possible
		move = block(g, p);
		if(move!=-1 && answercnt < g.remain_muncher_num) addanswer(move, p, answercnt, answer);
...
int make_move(int& move, graph& g, string& p, int depth, string&answer, int& answercnt){
	if(g.ad_remain_muncher_num == 0)
		...
		move = find(g, p, 2);
	answerlist[answerlistcount++] = move;
	for(int i=0; i<answerlistcount; ++i){
		g.nodes[answerlist[i]].status = NEWNUMCHER;
	}
	addanswer(move, p, answercnt, answer);
	g.clearmark();
	DFS(g, depth);//default 5
	test_route(g);
	for(int i=0; i<answerlistcount; ++i){
		g.nodes[answerlist[i]].status = ALIVE;
	}
	return 1;
}
